...
import {PieceSearchQuery} from '../../../piece/piece.searchquery';
import {PieceSelectionDialogComponent} from '../../../piece/selection-dialog/piece-selection-dialog.component';

@Component({
  selector: 'app-machine-view-piece',
  imports: [
    TableComponent
  ],
  templateUrl: './machine-view-piece.component.html',
  styleUrl: './machine-view-piece.component.scss'
})
export class MachineViewPieceComponent {
  ...
  protected readonly actionColumnInfo: ActionColumnInfo = {
    ...
    actions: [
      {name: "Retirer la pièce de la machine", action: this.unlinkPiece.bind(this), condition: _ => this.canEdit() }
    ],
    buttons: [
      {name: "Ajouter", action: this.linkPiece.bind(this), condition: () => this.canEdit() }
    ]
  };

  public readonly machine: InputSignal<Machine> = input.required();
  public readonly machineChanged: OutputEmitterRef<Machine> = output();
  ...
  /**
   * Ajoute une pièce à la machine
   */
  protected linkPiece(): Observable<boolean> {
    return this.matDialog
      ...
      .afterClosed()
      .pipe(
        map((returnedMachine: Machine | null): boolean => {
          if (!returnedMachine) {
            return false;
          }

          this.machineChanged.emit(returnedMachine);
          return true;
        }));
  }
